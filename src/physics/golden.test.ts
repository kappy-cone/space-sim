...
/** Reference ascent: the default craft flown by the ascent autopilot on a
 * fixed 0.25 s control cadence, sampled every 10 s. */
function runAscent(): Sample[] {
  ...
  for (let tick = 1; tick <= 2_400 && ap.phase !== 'failed'; tick++) {
    ap.update(sim);
    sim.step(0.25);
